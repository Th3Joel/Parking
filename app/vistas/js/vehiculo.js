var url = "controller/vehiculo.controller.php";
      ...
      d.map(x => {
        cont++;
          ren += "<tr>"+
                      ...
                      "<td>"+x.Color+"</td>"+
                      "<td>"+
                       "<button class='btn btn-primary' idEdit="+x.Placa+" id='btnEdit'><i class='bi bi-pen'></i></button>"+
                       "<button class='btn btn-danger' idElim="+x.Placa+" id='btnElim'><i class='bi bi-trash'></i></button>"+
                      "</td>"
      });
      if(d.length === 0){
...
$("#txtSearch").on("keyup",function(){
    datosTablas().TCar($("#txtSearch").val());
});
...
$("tbody").on("click","#btnEdit",function(){
    crud().get($(this).attr("idEdit"),1).then(d => {
        $("#txtPlaca").val(d.Placa);
        $("#txtCliente").val(d.Id_Cliente);
        $("#txtClasificacion").val(d.Id_Clasificacion);
        $("#txtMarca").val(d.Marca);
        $("#txtModelo").val(d.Modelo);
        $("#txtColor").val(d.Color);
        $("#btnEditModal").attr("idEdit",d.Placa)
